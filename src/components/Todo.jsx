...
export default function Todo() {
    ...
    const updafunc = (item) => {
        const updatetext = prompt(item.text)
        dispatch(updateTodo({ id: item.id, text: item.text, updatetext }))
    }
            ...
            {todo.map((todo) => (
                ...
                >
                    ...
                    <div className='space-x-3'>
                        <button
                            onClick={() => updafunc(todo)}
                            className="text-white bg-red-500 border-0 py-1 px-4 focus:outline-none hover:bg-red-600 rounded text-md"
                        >
                            <img src={updateImage} alt="update image" className='w-6' />
                        </button>
                        <button
